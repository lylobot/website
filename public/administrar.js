...
function addCommandRole(roleId = '') {
  const row = document.createElement('div');
  row.className = 'dynamic-row command-role-row';
  row.innerHTML = '<select name="ticketModerationRoleId"></select><button class="remove-button" type="button" aria-label="Borrar rol">×</button>';
  optionList(row.querySelector('select'), window.guildRoles, 'Seleccionar rol', roleId);
  row.querySelector('button').onclick = () => row.remove();
  document.querySelector('[data-command-role-list]').append(row);
}
...
function previewContainer(element, content, color, buttons = []) {
  ...
  element.innerHTML = `<div class="preview-brand"><img src="/bot-image.png" alt=""><span>Lylo Bot</span><small>APP</small></div><div class="preview-copy">${escapePreview(content).replace(/\n/g, '<br>')}</div>${buttons.length ? `<div class="preview-buttons">${buttons.map((button) => `<button type="button">${previewEmoji(button.emoji)} ${escapePreview(button.label || 'Abrir ticket')}</button>`).join('')}</div>` : ''}`;
}
...
async function load() {
  ...
  (config.ticketModerationRoleIds?.length ? config.ticketModerationRoleIds : ['']).forEach(addCommandRole);
...
form.onsubmit = async (event) => {
  ...
  const ticketModerationRoleIds = [...document.querySelectorAll('[name="ticketModerationRoleId"]')].map((element) => element.value).filter(Boolean);
  const payload = { guildId, welcome: { enabled: form.welcomeEnabled.checked, channelId: form.welcomeChannelId.value, roleId: form.welcomeRoleId.value, message: form.welcomeMessage.value, colorEnabled: form.welcomeColorEnabled.checked, color: form.welcomeColor.value, separator: true }, panelTitle: form.panelTitle.value, panelDescription: form.panelDescription.value, panelColor: form.panelColor.value, staffRoleId: form.staffRoleId.value, founderRoleId: founderRoleSelect.value, ticketModerationRoleIds, logChannelId: form.logChannelId.value, ticketQuestion: form.ticketQuestion.value, ticketModal: form.ticketModal.checked, ticketColorEnabled: form.ticketColorEnabled.checked, ticketColor: form.ticketColor.value, ticketCloseLog: form.ticketCloseLog.value, ticketCloseLogColorEnabled: form.ticketCloseLogColorEnabled.checked, ticketCloseLogColor: form.ticketCloseLogColor.value, buttons };
