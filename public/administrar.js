...
async function load() {
  ...
  const response = await fetch(`/api/guild-config?guildId=${encodeURIComponent(guildId)}`, { cache: 'no-store' });
  ...
  if (!response.ok) { const error = await response.json().catch(() => ({})); status.textContent = error.error || 'No se puede cargar la configuración.'; form.querySelectorAll('input, textarea, select, button').forEach((element) => { element.disabled = true; }); return; }
...
form.onsubmit = async (event) => {
  ...
  const response = await fetch('/api/guild-config', { method: 'PUT', cache: 'no-store', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
