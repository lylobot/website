...
function upsertLinkedUser(user, linkedAt) { const users = readJson(linkedUsersFile, []); const next = { id: user.id, username: user.global_name || user.username, tag: user.username, createdAt: snowflakeDate(user.id).toISOString(), linkedAt }; const index = users.findIndex((item) => item.id === user.id); if (index >= 0) users[index] = { ...users[index], ...next }; else users.push(next); saveJson(linkedUsersFile, users); }
...
async function getManageableGuild(session, guildId) {
  ...
  if (!guild || ((Number(guild.permissions) & 0x20) === 0 && !guild.owner)) throw new Error('Not allowed');
  return guild;
}
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/auth/callback') {
      ...
      upsertLinkedUser(user, linkedAt);
    ...
    if (url.pathname === '/api/control/users') { if (!adminAuthorized(request)) return controlError(response); const users = readJson(linkedUsersFile, []); const botGuilds = await getBotGuilds(); return sendJson(response, 200, { users: users.map((user) => ({ ...user, guilds: botGuilds.map((guild) => ({ name: guild.name, link: guild.vanity_url_code ? `https://discord.gg/${guild.vanity_url_code}` : `https://discord.com/channels/${guild.id}` })) })) }); }
    ...
    if (url.pathname === '/api/guilds') {
      ...
      return sendJson(response, 200, { guilds: userGuilds.filter((guild) => botIds.has(guild.id)) });
    }
