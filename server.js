...
const BOT_SYNC_SECRET = process.env.LYLO_BOT_SYNC_SECRET || '';
const dataDirectory = path.join(__dirname, 'data');
const sharedDataDirectory = process.env.LYLO_SHARED_DATA_DIR ? path.resolve(process.env.LYLO_SHARED_DATA_DIR) : path.join(__dirname, '..', 'data');
const sessionFile = path.join(dataDirectory, 'sessions.json');
const linkedUsersFile = path.join(dataDirectory, 'linked-users.json');
const activationFile = path.join(sharedDataDirectory, 'bot-activations.json');
const configFile = path.join(sharedDataDirectory, 'guild-config.json');
const premiumKeysFile = path.join(sharedDataDirectory, 'premium-keys.json');
const premiumProfilesFile = path.join(sharedDataDirectory, 'premium-profiles.json');
...
if (!fs.existsSync(premiumKeysFile)) fs.writeFileSync(premiumKeysFile, '[]');
...
function readPremiumKeys() {
  try { return JSON.parse(fs.readFileSync(premiumKeysFile, 'utf8')); } catch { return []; }
}

function savePremiumKeys(keys) {
  fs.writeFileSync(premiumKeysFile, JSON.stringify(keys, null, 2));
}

function premiumDuration(amount, unit) {
  const units = { seconds: 1000, minutes: 60000, hours: 3600000, months: 30 * 86400000, years: 365 * 86400000 };
  const number = Number(amount);
  if (!Number.isInteger(number) || number < 1 || number > 9999 || !units[unit]) return null;
  return { amount: number, unit, ms: number * units[unit], label: `${number} ${unit.toUpperCase()}` };
}

function prunePremiumKeys() {
  const now = Date.now();
  const keys = readPremiumKeys().filter((item) => item.used ? item.expiresAt > now : (item.deleteAt || (new Date(item.createdAt).getTime() + 48 * 60 * 60 * 1000)) > now);
...
async function getManageableGuild(session, guildId) {
  if (!/^\d+$/.test(String(guildId))) throw new Error('Not allowed');
  const guilds = await getUserGuilds(session.token);
  const guild = guilds.find((item) => item.id === guildId);
  if (!guild || !guild.owner) throw new Error('Not allowed');
  const botGuilds = await getBotGuilds();
  if (!botGuilds.some((item) => item.id === guildId)) throw new Error('Bot not installed');
  return guild;
}

function botApi(pathname, options = {}) {
  const { body, ...requestOptions } = options;
  return discordRequest(`https://discord.com/api/v10${pathname}`, { ...requestOptions, headers: { Authorization: `Bot ${BOT_TOKEN}`, ...(requestOptions.headers || {}) } }, body);
}
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/api/control/servers') {
      if (!adminAuthorized(request)) return controlError(response);
      const servers = readJson(activationFile, []);
      const botGuilds = await getBotGuilds();
      const botById = new Map(botGuilds.map((guild) => [guild.id, guild]));
      const linkedById = new Map(readJson(linkedUsersFile, []).map((user) => [user.id, user]));
      return sendJson(response, 200, {
        servers: servers.map((server) => {
          const liveGuild = botById.get(server.id);
          const owner = linkedById.get(server.ownerId);
          return {
            ...server,
            name: liveGuild?.name || server.name,
            ownerName: server.ownerName || owner?.username || '',
            user: server.ownerName || owner?.username || server.user || server.ownerId || 'No disponible',
            premiumSubscriptionCount: liveGuild?.premium_subscription_count || liveGuild?.premiumSubscriptionCount || server.premiumSubscriptionCount || 0
          };
        })
      });
    }
    if (url.pathname.startsWith('/api/control/servers/') && request.method === 'DELETE') {
      if (!adminAuthorized(request)) return controlError(response);
      const guildId = url.pathname.split('/').pop();
      if (!/^\d+$/.test(guildId)) return sendJson(response, 400, { error: 'Servidor inválido.' });
      try {
        await botApi(`/users/@me/guilds/${guildId}`, { method: 'DELETE' });
      } catch (error) {
        if (!/404|unknown guild|not found|503|unavailable/i.test(error.message)) {
          console.error(`No se pudo sacar Lylo de ${guildId}:`, error.message);
          return sendJson(response, 502, { error: `Discord no pudo completar la expulsión: ${error.message}` });
        }
      }
      const records = readJson(activationFile, []).filter((server) => server.id !== guildId);
      saveJson(activationFile, records);
      const config = readGuildConfig();
      if (config[guildId]) { delete config[guildId]; saveGuildConfig(config); }
      return sendJson(response, 200, { removed: guildId });
    }
    if (url.pathname.startsWith('/api/control/servers/') && url.pathname.endsWith('/reset') && request.method === 'POST') {
      if (!adminAuthorized(request)) return controlError(response);
      const parts = url.pathname.split('/');
      const guildId = parts[parts.length - 2];
      if (!/^\d+$/.test(guildId)) return sendJson(response, 400, { error: 'Servidor inválido.' });
      const config = readGuildConfig();
      config[guildId] = { activated: true };
      saveGuildConfig(config);
      return sendJson(response, 200, { reset: guildId });
    }
    if (url.pathname === '/api/control/reset' && request.method === 'POST') {
      if (!adminAuthorized(request)) return controlError(response);
      const config = readGuildConfig();
      const botGuilds = await getBotGuilds();
      for (const guild of botGuilds) config[guild.id] = { activated: true };
      for (const guildId of Object.keys(config)) config[guildId] = { activated: true };
      saveGuildConfig(config);
      return sendJson(response, 200, { reset: Object.keys(config).length });
    }
    if (url.pathname === '/api/control/keys' && request.method === 'GET') {
      if (!adminAuthorized(request)) return controlError(response);
      return sendJson(response, 200, { keys: prunePremiumKeys() });
    }
    if (url.pathname === '/api/internal/premium/claim' && request.method === 'POST') {
      if (!BOT_SYNC_SECRET || request.headers['x-lylo-sync-secret'] !== BOT_SYNC_SECRET) return sendJson(response, 401, { error: 'No autorizado.' });
      const body = await readBody(request);
      const keyValue = String(body.key || '').trim().toUpperCase();
      const guild = body.guild || {};
      if (!keyValue || !/^\d+$/.test(String(guild.id || ''))) return sendJson(response, 400, { error: 'Datos de activación inválidos.' });
      const keys = prunePremiumKeys();
      const item = keys.find((entry) => entry.key === keyValue);
      if (!item) return sendJson(response, 404, { error: 'La key no existe o ya expiró.' });
      if (item.used) return sendJson(response, 409, { error: 'La key ya está en uso.' });
      item.used = true;
      item.guildId = String(guild.id);
      item.guildName = String(guild.name || '').slice(0, 100);
      item.ownerId = String(guild.ownerId || '');
      item.usedAt = new Date().toISOString();
      item.expiresAt = Date.now() + premiumDuration(item.amount, item.unit).ms;
      savePremiumKeys(keys);
      return sendJson(response, 200, { item });
    }
    if (url.pathname === '/api/control/keys' && request.method === 'POST') {
      if (!adminAuthorized(request)) return controlError(response);
      const body = await readBody(request);
      const duration = premiumDuration(body.amount, body.unit);
      if (!duration) return sendJson(response, 400, { error: 'Duración inválida.' });
      const keys = prunePremiumKeys();
      const item = { key: generatePremiumKey(), amount: duration.amount, unit: duration.unit, durationLabel: duration.label, createdAt: new Date().toISOString(), deleteAt: Date.now() + 48 * 60 * 60 * 1000, expiresAt: null, used: false, guildId: null, guildName: null, ownerId: null, usedAt: null };
      keys.push(item);
      savePremiumKeys(keys);
      return sendJson(response, 201, { key: item });
    }
    if (url.pathname.startsWith('/api/control/keys/') && url.pathname.endsWith('/revoke') && request.method === 'POST') {
      if (!adminAuthorized(request)) return controlError(response);
      const keyValue = decodeURIComponent(url.pathname.split('/').slice(-2, -1)[0]).toUpperCase();
      const keys = prunePremiumKeys();
      const item = keys.find((entry) => entry.key === keyValue);
      if (!item || !item.used || !item.guildId) return sendJson(response, 404, { error: 'Key activa no encontrada.' });
      const profiles = readJson(premiumProfilesFile, {});
      const profile = profiles[item.guildId];
      if (profile?.original) {
        try {
          const headers = { 'Content-Type': 'application/json', 'X-Audit-Log-Reason': 'Premium revoked' };
          await botApi(`/guilds/${item.guildId}/members/@me`, { method: 'PATCH', headers, body: JSON.stringify({ nick: profile.original.nick ?? null }) });
          const visualProfile = {};
          if (profile.original.avatar) visualProfile.avatar = profile.original.avatar;
          if (profile.original.banner) visualProfile.banner = profile.original.banner;
          if (profile.original.bio !== undefined) visualProfile.bio = profile.original.bio;
          if (Object.keys(visualProfile).length) await botApi(`/guilds/${item.guildId}/members/@me`, { method: 'PATCH', headers, body: JSON.stringify(visualProfile) });
        } catch (error) {
          return sendJson(response, 502, { error: `No se pudo restaurar el perfil: ${error.message}` });
        }
        delete profiles[item.guildId];
        saveJson(premiumProfilesFile, profiles);
      }
      item.revokedAt = new Date().toISOString();
      item.revoked = true;
      item.expiresAt = Date.now();
      savePremiumKeys(keys);
      return sendJson(response, 200, { revoked: item.key });
    }

    if (url.pathname === '/api/public-guilds') {
      const guilds = await discordRequest('https://discord.com/api/v10/users/@me/guilds?with_counts=true', { headers: { Authorization: `Bot ${BOT_TOKEN}` } });
      return sendJson(response, 200, { guilds: guilds.map((guild) => ({ id: guild.id, name: guild.name, icon: guild.icon || null, memberCount: Number(guild.approximate_member_count) || 0 })) });
    }
    ...
    if (url.pathname === '/api/guild-config' && request.method === 'PUT') {
      ...
      config[body.guildId] = {
        ...
        commandRoleIds: body.commandRoleIds && typeof body.commandRoleIds === 'object' ? Object.fromEntries(Object.entries(body.commandRoleIds).filter(([name, roleId]) => /^[a-z0-9-]+$/.test(name) && String(roleId).trim()).slice(0, 50).map(([name, roleId]) => [name, String(roleId).slice(0, 30)])) : (previous.commandRoleIds || {}),
    ...
    if (error.message === 'Bot not installed') return sendJson(response, 403, { error: 'Lylo no está instalado en este servidor.' });
