...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/auth/callback') {
      ...
      const sessionCookie = createSession({ token: token.access_token, user });
    ...
    if (url.pathname === '/api/guild-config' && request.method === 'PUT') {
      ...
      config[body.guildId] = {
        ...
        ticketColor: ['blue', 'green', 'gold', 'pink'].includes(body.ticketColor) ? body.ticketColor : 'blue',
    ...
    redirect(response, '/?error=oauth_failed');
  }
});
