...
const premiumKeysFile = path.join(dataDirectory, 'premium-keys.json');
...
function prunePremiumKeys() {
  const keys = readPremiumKeys().filter((item) => item.expiresAt > Date.now());
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/api/control/keys' && request.method === 'POST') {
      ...
      const item = { key: generatePremiumKey(), amount: duration.amount, unit: duration.unit, durationLabel: duration.label, createdAt: new Date().toISOString(), expiresAt: Date.now() + duration.ms, used: false, guildId: null, guildName: null, ownerId: null, usedAt: null };
      keys.push(item);
      savePremiumKeys(keys);
    ...
    if (url.pathname.startsWith('/api/control/keys/') && url.pathname.endsWith('/revoke') && request.method === 'POST') {
      ...
      const profiles = readJson(path.join(dataDirectory, 'premium-profiles.json'), {});
      ...
      if (profile?.original) {
        ...
        saveJson(path.join(dataDirectory, 'premium-profiles.json'), profiles);
      }
